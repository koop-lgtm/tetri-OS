const dbRoot = 'https://eggborne.com';
...
function laneBeingTouched() {
  var distanceFromLeft =
    Math.abs(cursor.x - (pieceBG.x + pieceBuffer)) - blockSize / 2;

  var actualLane = Math.round(distanceFromLeft / blockSize);
  if (
    (atBat.type === 'zee' || atBat.type === 'ess') &&
    (atBat.position === 'right' || atBat.position === 'left')
  ) {
    //        actualLane--;
  }
  if (
...
function getActualCoords(origX, origY) {
  ...
  return {
    x: adjScale * (origX - currentSize.x / 2) + currentSize.x / 2,
    y: adjScale * (origY - currentSize.y / 2) + currentSize.y / 2
  };
}
...
function checkKeyInputs() {
  if (
    (counter - lastLanded >= ARETime * 2 && !atBat.forced && pressingS) ||
    pressedSAt === counter ||
    atBat.droppedAt === counter
  ) {
    atBat.forced = true;
  ...
  if (pressingA) {
    ...
    if (
      pressedAAt === counter ||
      sincePressed === DASTime ||
      (sincePressed > DASTime && sincePressed % DASInterval === 0)
    ) {
      atBat.shift(-1, -1);
    }
  }
...
function getScoresFromDatabase(gameName, populate, display) {
  ...
  $.ajax({
    type: 'get',
    url: dbRoot + '/scripts/getscores.php',
    data: { game: gameName },
    ...
    success: function(text) {
      ...
      if (!populate) {
        ...
        if (scoreArray.length < 10) {
          console.log(
            'only ' + scoreArray.length + 'scores though, so score goes in!'
          );
          tenth = 0;
        }
...
function saveScoreToDatabase(gameName, playerName, playerScore, reset) {
  $.ajax({
    type: 'post',
    url: dbRoot + '/scripts/savescores.php',
...
function getTotalLinesFromDatabase() {
  $.ajax({
    type: 'get',
    url: dbRoot + '/scripts/getlines.php',
...
function sendLinesScoredToDatabase(numOfLines) {
  $.ajax({
    type: 'post',
    url: dbRoot + '/scripts/sendlinesscored.php',
